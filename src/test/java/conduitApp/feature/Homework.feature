...
Scenario: Favorite articles
    ...
    * print firstSlug
    * def firstFavorites = response.articles[0].favoritesCount
    * print firstFavorites 
    # Step 3: Make POST request to increse favorites count for the first article
    Given path 'articles', firstSlug, '/favorite'
    * configure headers = {Content-Length : 0}
    And request {}
    And header Authorization = 'Token ' + authToken
    When method POST
    Then status 200
    # Step 4: Verify response schema
    And match response contains firstFavorites == firstFavorites + 1
